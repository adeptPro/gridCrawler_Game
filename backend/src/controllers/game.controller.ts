...
export const saveGame = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const gameStateData = req.body as IGameState; 
    ...
    const updatedGameState = await GameStateModel.findOneAndUpdate(
      { playerName: gameStateData.playerName }, // Query to find the document.
 ...
 * @returns {Promise<void>} A promise that resolves when the response has been sent.
 */
export const loadGame = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const gameState = await GameStateModel.findOne({ playerName });
